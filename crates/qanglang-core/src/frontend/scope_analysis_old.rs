...
impl FunctionContext {
    fn new(
        ...
        // Initialize based on function kind to match old compiler behavior
        let local_count = match kind {
            FunctionKind::Method | FunctionKind::Initializer => {
                ...
                locals.push(LocalVariable {
                    name: blank_handle, // Will be set to "this" later
                    scope_depth: 0,
                    slot: 0,
                    is_captured: false,
                    is_initialized: true, // 'this' is immediately available
                });
                1 // Parameters start at slot 1
            }
            _ => {
                // For regular functions and globals, slot 0 is just a placeholder
                locals.push(LocalVariable {
                    name: blank_handle,
                    scope_depth: 0,
                    slot: 0,
                ...
                1 // Parameters start at slot 1 for regular functions too (to match old compiler)
            }
        };
    ...
    fn add_local(&mut self, name: StringHandle, scope_depth: usize, is_initialized: bool) -> (usize, usize) {
    ...
    fn declare_variable_with_init(
        ...
        let (variable_info, local_index) = if self.current_scope_depth() == 0 {
            ...
            let (local_idx, slot) = if let Some(function) = self.functions.last_mut() {
                function.add_local(name, scope_depth, is_initialized)
            } else {
                (0, 0)
            };
        ...
        // Add to results
        self.results.variables.insert(identifier.id, variable_info.clone());

        // DEBUG: Print what we're storing in scope analysis
        if self.current_scope_depth() > 0 {
            if let VariableKind::Local { slot, .. } = &variable_info.kind {
                let var_name = self.strings.get_string(name);
                println!("DEBUG SCOPE: Storing variable - node_id: {:?}, name: '{}', slot: {}, scope_depth: {}",
                    identifier.id, var_name, slot, self.current_scope_depth());
            }
        }
    ...
    fn resolve_variable(
        ...
        if let Some(function) = self.functions.last() {
            ...
            if let Some(local_index) = function.find_local(name, current_scope_depth) {
                ...
                // DEBUG: Print variable reference resolution
                // if let VariableKind::Local { slot, .. } = &variable_kind {
                //     let var_name = self.strings.get_string(name);
                //     println!("DEBUG SCOPE: Resolving variable reference - node_id: {:?}, name: '{}', slot: {}",
                //         node_id, var_name, slot);
                // }
    ...
    fn visit_class_declaration(
        ...
        if has_superclass {
            ...
            let super_slot = if let Some(func) = self.functions.last_mut() {
                let (_local_index, slot) = func.add_local(super_handle, scope_depth, true); // Super should be immediately initialized
                slot
            } else {
                ctx.errors
                    .report_error(QangCompilerError::new_analysis_error(
