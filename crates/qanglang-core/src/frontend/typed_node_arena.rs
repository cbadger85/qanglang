...
impl TypedNodeArena {
    ...
    pub fn get_decl_node(&self, node_id: NodeId) -> DeclNode {
        let node = self.nodes[node_id.get()];
        node.try_into().unwrap()
    }

    pub fn get_stmt_node(&self, node_id: NodeId) -> StmtNode {
        let node = self.nodes[node_id.get()];
        node.try_into().unwrap()
    }

    pub fn get_expr_node(&self, node_id: NodeId) -> ExprNode {
        let node = self.nodes[node_id.get()];
        node.try_into().unwrap()
    }

    pub fn get_call_operation_node(&self, node_id: NodeId) -> CallOperationNode {
        let node = self.nodes[node_id.get()];
        node.try_into().unwrap()
    }

    pub fn get_assignment_target_node(&self, node_id: NodeId) -> AssignmentTargetNode {
        let node = self.nodes[node_id.get()];
        node.try_into().unwrap()
    }

    pub fn get_primary_node(&self, node_id: NodeId) -> PrimaryNode {
        let node = self.nodes[node_id.get()];
        node.try_into().unwrap()
    }

    pub fn get_var_decl_node(&self, node_id: NodeId) -> VariableDeclNode {
        let node = self.nodes[node_id.get()];

        match node {
            AstNode::VariableDecl(decl) => decl,
            _ => panic!("Expected VariableDeclNode."),
        }
    }

    pub fn get_program_node(&self, node_id: NodeId) -> ProgramNode {
        let node = self.nodes[node_id.get()];
    ...
    pub fn get_identifier_node(&self, node_id: NodeId) -> IdentifierNode {
        let node = self.nodes[node_id.get()];
        match node {
    ...
    pub fn get_func_expr_node(&self, node_id: NodeId) -> FunctionExprNode {
        let node = self.nodes[node_id.get()];

        match node {
            AstNode::FunctionExpr(func) => func,
            _ => panic!("Expected FunctionExprNode"),
        }
    }

    pub fn get_lambda_expr_node(&self, node_id: NodeId) -> LambdaExprNode {
        let node = self.nodes[node_id.get()];

        match node {
            AstNode::LambdaExpr(lambda) => lambda,
            _ => panic!("Expected LambdaExprNode"),
        }
    }

    pub fn get_block_stmt_node(&self, node_id: NodeId) -> BlockStmtNode {
        let node = self.nodes[node_id.get()];

        match node {
            AstNode::BlockStmt(block) => block,
            _ => panic!("Expected BlockStmtNode."),
        }
    }

    pub fn get_lambda_body_node(&self, node_id: NodeId) -> LambdaBodyNode {
        let node = self.nodes[node_id.get()];
        node.try_into().unwrap()
    }

    pub fn get_for_initializer_node(&self, node_id: NodeId) -> ForInitializerNode {
        let node = self.nodes[node_id.get()];
        node.try_into().unwrap()
    }

    pub fn iter_decl_nodes(array_id: NodeArrayId) {
        // TODO iterate over the array's node ids, supplying the ones that are declarations. Assume the parser correctly added the right nodes.
        todo!()
    }

    pub fn iter_expr_nodes(array_id: NodeArrayId) {
        // TODO iterate over the array's node ids, supplying the ones that are expressions. Assume the parser correctly added the right nodes.
        todo!()
    }

    pub fn iter_object_entry_nodes(array_id: NodeArrayId) {
        // TODO iterate over the array's node ids, supplying the ones that are object entries. Assume the parser correctly added the right nodes.
        todo!()
    }

    pub fn iter_class_member_nodes(array_id: NodeArrayId) {
        // TODO iterate over the array's node ids, supplying the ones that are class members. Assume the parser correctly added the right nodes.
        todo!()
    }

    pub fn iter_identifier_nodes(array_id: NodeArrayId) {
        // TODO iterate over the array's node ids, supplying the ones that are identifiers. Assume the parser correctly added the right nodes.
        todo!()
    }
}
