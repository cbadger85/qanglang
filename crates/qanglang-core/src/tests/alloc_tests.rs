#[cfg(test)]
mod tests {
    use crate::memory::{HeapAllocator, ClosureObject, FunctionObject, UpvalueReference};
    use crate::Value;
    ...
    fn test_mark_closure() {
        ...
        let handle = allocator.allocate_closure(closure);
        assert!(!allocator.get_closure(handle).is_marked);
        
        allocator.mark_closure(handle);
        assert!(allocator.get_closure(handle).is_marked);
    }
    ...
    #[test]
    fn test_mark_upvalue() {
        let mut allocator = HeapAllocator::new();
        let value = Value::Number(42.0);
        
        let handle = allocator.allocate_upvalue(value);
        allocator.mark_upvalue(handle);
        
        // Upvalue should be marked (we can't directly check this without accessing internal state)
        // But we can test that it survives garbage collection
    }
    ...
    fn test_mark_class() {
        ...
        let handle = allocator.allocate_class(class_name);
        assert!(!allocator.get_class(handle).is_marked);
        
        allocator.mark_class(handle);
        assert!(allocator.get_class(handle).is_marked);
    }
    ...
    #[test]
    fn test_mark_instance() {
        let mut allocator = HeapAllocator::new();
        let class_name = allocator.strings.intern("TestClass");
        let class_handle = allocator.allocate_class(class_name);
        let instance_handle = allocator.allocate_instance(class_handle);
        
        assert!(!allocator.get_instance(instance_handle).is_marked);
        
        allocator.mark_instance(instance_handle);
        assert!(allocator.get_instance(instance_handle).is_marked);
    }
    ...
    fn test_total_allocated_bytes_calculation() {
        ...
        assert!(bytes_increase > 100, // Should be much larger than 100 bytes with proper multiplication
                "Byte increase should be substantial with proper calculation. Got: {}", bytes_increase);
    }
    ...
    fn test_class_and_instance_gc() {
        ...
        let field_name = allocator.strings.intern("field1");
        allocator.set_instance_field(instance_handle, Value::String(field_name), Value::Number(42.0));
    ...
    fn test_gc_preserves_instance_class_relationship() {
        ...
        allocator.set_instance_field(instance_handle, Value::String(field_name), Value::Closure(field_value_closure_handle));
        ...
        let field_value = allocator.get_instance_field(instance_handle, Value::String(field_name));
        assert_eq!(field_value, Some(Value::Closure(field_value_closure_handle)));
    ...
    fn test_table_references_keep_objects_alive_during_gc() {
        ...
        allocator.set_instance_field(instance1_handle, Value::String(field_name1), Value::Closure(method1_closure_handle));
        allocator.set_instance_field(instance1_handle, Value::String(field_name2), Value::Closure(method2_closure_handle));
        allocator.set_instance_field(instance1_handle, Value::String(field_name3), Value::Instance(instance2_handle));
        allocator.set_instance_field(instance2_handle, Value::String(field_name3), Value::Instance(instance1_handle));
        ...
        allocator.set_instance_field(orphaned_instance_handle, Value::String(orphaned_field), Value::Number(999.0));
        ...
        let method1_field = allocator.get_instance_field(instance1_handle, Value::String(field_name1));
        assert_eq!(method1_field, Some(Value::Closure(method1_closure_handle)));
        
        let method2_field = allocator.get_instance_field(instance1_handle, Value::String(field_name2));
        assert_eq!(method2_field, Some(Value::Closure(method2_closure_handle)));
        
        let sibling_field = allocator.get_instance_field(instance1_handle, Value::String(field_name3));
        assert_eq!(sibling_field, Some(Value::Instance(instance2_handle)));
        
        let reverse_sibling = allocator.get_instance_field(instance2_handle, Value::String(field_name3));
    ...
    fn test_table_objects_collected_when_no_references() {
        ...
        for i in 0..3 {
            ...
            allocator.set_instance_field(instance_handle, Value::String(field_name), Value::Closure(closure_handle));
        }
        ...
        assert!(bytes_collected > 200, // Should be much more than just a few objects
                "Expected substantial bytes to be collected. Only collected: {}", bytes_collected);
    }
    ...
    fn test_objects_in_tables_survive_gc_when_reachable() {
        ...
        allocator.set_instance_field(container_instance, Value::String(closure_field), Value::Closure(target_closure_handle));
        allocator.set_instance_field(container_instance, Value::String(instance_field), Value::Instance(target_instance_handle));
        allocator.set_instance_field(container_instance, Value::String(class_field), Value::Class(target_class_handle));
        ...
        let closure_ref = allocator.get_instance_field(container_instance, Value::String(closure_field));
        assert_eq!(closure_ref, Some(Value::Closure(target_closure_handle)),
                   "Closure referenced by table should still be accessible");
        
        let instance_ref = allocator.get_instance_field(container_instance, Value::String(instance_field));
        assert_eq!(instance_ref, Some(Value::Instance(target_instance_handle)),
                   "Instance referenced by table should still be accessible");
        
        let class_ref = allocator.get_instance_field(container_instance, Value::String(class_field));
        assert_eq!(class_ref, Some(Value::Class(target_class_handle)),
    ...
    fn test_complex_mixed_object_scenario() {
        ...
        allocator.set_instance_field(base_instance, Value::String(field_name1), Value::Instance(derived_instance));
        allocator.set_instance_field(derived_instance, Value::String(field_name1), Value::Instance(base_instance));
        allocator.set_instance_field(base_instance, Value::String(field_name2), Value::Closure(closure_handle1));
        ...
        let derived_ref = allocator.get_instance_field(derived_instance, Value::String(field_name1));
