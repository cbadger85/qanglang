...
The logical operators `and` and `or` can be used to apply multiple conditions.

````ql
if (first_condition and second_condition) { // Is true if both first_condition and second_condition are true
  // Do something
} else if (first_condition or second_condition) { // Is true if first_condition or second_condition is true
  // Do something else
}
