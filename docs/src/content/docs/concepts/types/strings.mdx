...
There is no implicit conversion of strings in QangLang. When using the `+` operator to concatenate, ensure both values are strings by using the [`to_string` function](/reference/api/global-functions#to_string).
