...
assert_eq(1, 0, "This is going to error."); // Assertion is false, the program exits, giving an error using the message if provided.
...
assert_throws(println("Hello World", "This is going to error.")); // Assertion is false because this does not error. The program exits, giving an error using the message if provided.
