...
Removes the first item of the array, mutating it and returns it.

```ql
var first = [1, 2].pop_front(); // 1
```
...
`dyn!` - The first element of the array. `nil` if the array is empty.
...
Adds a new item to the front of the array, mutating it.

```ql
var arr = [1];
arr.push_front(0); // [0, 1]
```
