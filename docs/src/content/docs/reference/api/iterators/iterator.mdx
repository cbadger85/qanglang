...
Skips a number of items from the beginning of the iterator until the predicate returns true.

```ql
var iter = Range(0, 10).skip_until((n) -> n <= 5);
```
...
`SkipUntilIterator` - A [`SkipUntilIterator`](/reference/api/iterators/skip-until-iterator/) for skipping the items matching the predicate.
...
Takes a number of items from the beginning of the iterator until the predicate returns true.

```ql
var iter = Range(0, 10).take_until((n) -> n > 5);
```
...
`TakeUntilIterator` - A [`TakeUntilIterator`](/reference/api/iterators/take-until-iterator/) for taking the items matching the predicate.
...
Collects an array of [entries](/reference/api/entry/) to a map.
...
`HashMap` - An [`HashMap`](/reference/api/hashmap/) of the entries.

### `to_set`

Collects an array values into a set.

```ql
var arr = [1, 1, 2, 2, 3];
var map = arr.iter().to_set();
...
`HashSet` - An [`HashSet`](/reference/api/hashset/) of the values.
