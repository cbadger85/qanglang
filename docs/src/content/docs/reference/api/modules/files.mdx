...
Creates an iterator to read a file incrementally as chunks of strings.
...
var file_path = path.Path("./big_file.txt");

// Print the contents of the file to the console
files.FileIterator(file_path).for_each(print);
```
...
`Result` - Returns `Ok` of `nil`, if successful, or `Err` if the path is invalid, the file doesn't exist, or the operation fails.
...
`Result` - Returns `Ok` of `nil` if successful, or `Err` if either path is invalid, source is not a file, or the operation fails.
...
`Result` - Returns `Ok` of `nil` if successful, or `Err` if the path is invalid or the operation fails.
...
`Result` - Returns `Ok` of the number of miliseconds between the Unix epoch and when the file was modified, or `Err` if the path is invalid or metadata cannot be retrieved.
...
`Result` - Returns `Ok` of the file size in bytes, or `Err` if the path is invalid or metadata cannot be retrieved.
...
`Result` - Returns `Ok` of an array of entry names, or `Err` if the path is invalid or the directory cannot be read.
...
`Result` - Returns `Ok` of `nil` if successful, or `Err` if either path is invalid or the operation fails.
...
`Result` - Returns `Ok` of a `FileIterator`, or `Err` if the path is invalid.
...
`Result` - Returns `Ok` of the file contents as a string, or `Err` if the path is invalid or the file cannot be read.
...
`Result` - Returns `Ok` of `nil` if successful, or `Err` if the path is invalid, the directory is not empty, or the operation fails.
...
`Result` - Returns `Ok` of `nil` if successful, or `Err` if the path is invalid, content is not a string, or the operation fails.
