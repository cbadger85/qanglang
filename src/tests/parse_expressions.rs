...
fn test_optional_chaining() {
    ...
    if let ast::Decl::Stmt(ast::Stmt::Expr(expr_stmt)) = &program.decls[0] {
        if let ast::Expr::Assignment(assignment) = &expr_stmt.expr {
            ...
            // Value should be optional property access obj.?property
            if let ast::Expr::Call(call_expr) = assignment.value.as_ref() {
                // Callee should be identifier 'obj'
                if let ast::Expr::Primary(ast::PrimaryExpr::Identifier(obj_id)) =
                    call_expr.callee.as_ref()
                ...
                // Operation should be optional property access to 'property'
                if let ast::CallOperation::OptionalProperty(property_id) =
                    call_expr.operation.as_ref()
                {
                    assert_eq!(property_id.name.as_ref(), "property");
                } else {
                    panic!("Expected optional property access to 'property'");
                }
            } else {
                panic!("Expected call expression for optional property access");
            }
        } else {
            panic!("Expected assignment expression");
        }
    ...
    // Second access: value2 = obj.?method().?result;
    if let ast::Decl::Stmt(ast::Stmt::Expr(expr_stmt)) = &program.decls[1] {
        if let ast::Expr::Assignment(assignment) = &expr_stmt.expr {
            // Target should be identifier 'value2'
            if let ast::AssignmentTarget::Identifier(target_id) = &assignment.target {
            ...
            // Value should be chained optional access: obj.?method().?result
            // This should be parsed as: ((obj.?method)()).?result
            if let ast::Expr::Call(final_call) = assignment.value.as_ref() {
                // Final operation should be optional property access to 'result'
                if let ast::CallOperation::OptionalProperty(result_id) =
                    final_call.operation.as_ref()
                {
                    assert_eq!(result_id.name.as_ref(), "result");
                } else {
                    panic!("Expected optional property access to 'result'");
                }

                // The callee should be obj.?method()
                if let ast::Expr::Call(method_call) = final_call.callee.as_ref() {
                    // This should be a function call operation with no arguments
                    if let ast::CallOperation::Call(args) = method_call.operation.as_ref() {
                        assert_eq!(args.len(), 0);
                    } else {
                        panic!("Expected call operation");
                    }

                    // The callee of the method call should be obj.?method
                    if let ast::Expr::Call(optional_access) = method_call.callee.as_ref() {
                        // Should be optional property access to 'method'
                        if let ast::CallOperation::OptionalProperty(method_id) =
                            optional_access.operation.as_ref()
                        {
                            assert_eq!(method_id.name.as_ref(), "method");
                        } else {
                            panic!("Expected optional property access to 'method'");
                        }

                        // The callee should be 'obj'
                        if let ast::Expr::Primary(ast::PrimaryExpr::Identifier(obj_id)) =
                            optional_access.callee.as_ref()
                        {
                            assert_eq!(obj_id.name.as_ref(), "obj");
                        } else {
                            panic!("Expected identifier 'obj'");
                        ...
                        panic!("Expected call expression for obj.?method");
                    }
                } else {
                    panic!("Expected call expression for method call");
                }
            } else {
...
fn test_complex_nested_expression() {
    ...
    if let ast::Decl::Variable(var_decl) = &program.decls[0] {
        ...
        // Expression: obj.method(a + b * c).property[index].?optional |> transform
        // This should parse as: (obj.method(a + b * c).property[index].?optional) |> transform
        if let Some(ast::Expr::Pipe(pipe_expr)) = &var_decl.initializer {
            // Right side should be identifier 'transform'
            assert!(pipe_expr.right.is_some());
            if let Some(right_expr) = &pipe_expr.right {
                if let ast::Expr::Primary(ast::PrimaryExpr::Identifier(transform_id)) =
                    right_expr.as_ref()
            ...
            // Left side: obj.method(a + b * c).property[index].?optional
            // This should be: ((((obj.method(a + b * c)).property)[index]).?optional)
            if let ast::Expr::Call(optional_call) = pipe_expr.left.as_ref() {
                // Final operation: .?optional
                if let ast::CallOperation::OptionalProperty(optional_id) =
                    optional_call.operation.as_ref()
                {
                    assert_eq!(optional_id.name.as_ref(), "optional");
                } else {
                    panic!("Expected optional property access to 'optional'");
                }

                // Callee: obj.method(a + b * c).property[index]
                if let ast::Expr::Call(index_call) = optional_call.callee.as_ref() {
                    // Operation: [index]
                    if let ast::CallOperation::Index(index_expr) = index_call.operation.as_ref() {
                        if let ast::Expr::Primary(ast::PrimaryExpr::Identifier(index_id)) =
                            index_expr
                        {
                    ...
                    // Callee: obj.method(a + b * c).property
                    if let ast::Expr::Call(property_call) = index_call.callee.as_ref() {
                        // Operation: .property
                        ...
                        // Callee: obj.method(a + b * c)
                        if let ast::Expr::Call(method_call) = property_call.callee.as_ref() {
                            // Operation: function call with argument (a + b * c)
                            if let ast::CallOperation::Call(args) = method_call.operation.as_ref() {
                                assert_eq!(args.len(), 1);

                                // Argument: a + b * c
                                if let ast::Expr::Term(term_expr) = &args[0] {
                                    ...
                                    {
                                        assert_eq!(
                                            factor_expr.operator,
                                            ast::FactorOperator::Multiply
                                        );

                                        // Left side: identifier 'b'
                                        if let ast::Expr::Primary(ast::PrimaryExpr::Identifier(
                                            b_id,
                                        )) = factor_expr.left.as_ref()
                                        {
                                            assert_eq!(b_id.name.as_ref(), "b");
                            ...
                            // Callee: obj.method
                            if let ast::Expr::Call(obj_method) = method_call.callee.as_ref() {
                                // Operation: .method
                                if let ast::CallOperation::Property(method_id) =
                                    obj_method.operation.as_ref()
                                {
                                    assert_eq!(method_id.name.as_ref(), "method");
                                } else {
                                    panic!("Expected property access to 'method'");
                    ...
                    panic!("Expected call expression for index access");
                }
            } else {
                panic!("Expected call expression for optional property access");
            }
