...
fn tokenize_all(source_map: Rc<SourceMap>) -> Vec<Token> {
...
fn assert_token_types(source: &str, expected: &[TokenType]) {
    ...
    let tokens = tokenize_all(source_map.clone());
...
fn test_unterminated_string() {
    let source_map = Rc::new(SourceMap::new("\"unterminated".to_string()));
    let tokens = tokenize_all(source_map.clone());
...
fn test_unterminated_string_with_following_code() {
    let source_map = Rc::new(SourceMap::new("\"unterminated\nvar x = 5;".to_string()));
...
fn test_unexpected_character() {
    let source_map = Rc::new(SourceMap::new("@".to_string()));
    let tokens = tokenize_all(source_map.clone());
    assert_eq!(tokens.len(), 2);
    match &tokens[0].token_type {
...
fn test_malformed_numbers() {
    ...
    println!(
        "{:?}",
        tokenize_all(Rc::new(SourceMap::new(source4.to_owned())))
    );
...
fn test_invalid_characters_in_different_contexts() {
    let source_map1 = Rc::new(SourceMap::new("@variable".to_string()));
    let tokens1 = tokenize_all(source_map1.clone());
    ...
    let source_map2 = Rc::new(SourceMap::new("var x = #123".to_string()));
    let tokens2 = tokenize_all(source_map2.clone());
    ...
    let source_map3 = Rc::new(SourceMap::new("func($arg)".to_string()));
    let tokens3 = tokenize_all(source_map3.clone());
    ...
    let source_map4 = Rc::new(SourceMap::new("var result = 10 & 5;".to_string()));
    let tokens4 = tokenize_all(source_map4.clone());
    ...
    let source_map5 = Rc::new(SourceMap::new("@#$".to_string()));
    let tokens5 = tokenize_all(source_map5.clone());
...
fn test_nested_unterminated_constructs() {
    let source_map1 = Rc::new(SourceMap::new(
        "\"unterminated string with 'single quotes'".to_string(),
    ));
    let tokens1 = tokenize_all(source_map1.clone());
    assert_eq!(tokens1[0].token_type, TokenType::Error);
    assert_eq!(
    ...
    let source_map2 = Rc::new(SourceMap::new(
    ...
    let source_map3 = Rc::new(SourceMap::new("/* unterminated with // inside".to_string()));
    let tokens3 = tokenize_all(source_map3.clone());
    ...
    let source_map4 = Rc::new(SourceMap::new(
        "/* outer comment /* nested start but no end".to_string(),
    ));
    let tokens4 = tokenize_all(source_map4.clone());
    ...
    let source_map5 = Rc::new(SourceMap::new(
        "\"escaped \\\" quote but still unterminated".to_string(),
    ));
    let tokens5 = tokenize_all(source_map5.clone());
    assert_eq!(tokens5[0].token_type, TokenType::Error);
    assert_eq!(
...
fn test_invalid_escape_sequences() {
    let source_map1 = Rc::new(SourceMap::new("\"hello\\qworld\"".to_string()));
    let tokens1 = tokenize_all(source_map1.clone());
    assert_eq!(tokens1[0].token_type, TokenType::Error);
    assert_eq!(
        tokens1[0].error_message.as_ref().unwrap(),
    ...
    let source_map2 = Rc::new(SourceMap::new("\"\\x invalid escape\"".to_string()));
    let tokens2 = tokenize_all(source_map2.clone());
    assert_eq!(tokens2[0].token_type, TokenType::Error);
    assert_eq!(
        tokens2[0].error_message.as_ref().unwrap(),
    ...
    let source_map3 = Rc::new(SourceMap::new("\"\\z another invalid\"".to_string()));
    let tokens3 = tokenize_all(source_map3.clone());
    assert_eq!(tokens3[0].token_type, TokenType::Error);
    assert_eq!(
        tokens3[0].error_message.as_ref().unwrap(),
...
fn test_unterminated_multiline_comment() {
    let source_map = Rc::new(SourceMap::new("/* unterminated comment".to_string()));
    let tokens = tokenize_all(source_map.clone());
...
fn test_number_lexeme_content() {
    let source_map1 = Rc::new(SourceMap::new("123".to_string()));
    let tokens1 = tokenize_all(source_map1.clone());
    ...
    let source_map2 = Rc::new(SourceMap::new("123.456".to_string()));
    let tokens2 = tokenize_all(source_map2.clone());
    ...
    let source_map3 = Rc::new(SourceMap::new(".789".to_string()));
    let tokens3 = tokenize_all(source_map3.clone());
    ...
    let source_map6 = Rc::new(SourceMap::new("0.0".to_string()));
    let tokens6 = tokenize_all(source_map6.clone());
...
fn test_string_lexeme_content() {
    let source_map1 = Rc::new(SourceMap::new("\"hello\"".to_string()));
    let tokens1 = tokenize_all(source_map1.clone());
    assert_eq!(tokens1[0].token_type, TokenType::String);
    let lexeme1: String = tokens1[0].lexeme(&source_map1).iter().collect();
    assert_eq!(lexeme1, "\"hello\"");

    let source_map2 = Rc::new(SourceMap::new("\"\"".to_string()));
    let tokens2 = tokenize_all(source_map2.clone());
    assert_eq!(tokens2[0].token_type, TokenType::String);
    let lexeme2: String = tokens2[0].lexeme(&source_map2).iter().collect();
    assert_eq!(lexeme2, "\"\"");

    let source_map3 = Rc::new(SourceMap::new("\"hello\\nworld\\t\"".to_string()));
    let tokens3 = tokenize_all(source_map3.clone());
    assert_eq!(tokens3[0].token_type, TokenType::String);
    let lexeme3: String = tokens3[0].lexeme(&source_map3).iter().collect();
    assert_eq!(lexeme3, "\"hello\\nworld\\t\"");

    let source_map4 = Rc::new(SourceMap::new("\"say \\\"hello\\\"\"".to_string()));
    let tokens4 = tokenize_all(source_map4.clone());
    assert_eq!(tokens4[0].token_type, TokenType::String);
    let lexeme4: String = tokens4[0].lexeme(&source_map4).iter().collect();
    assert_eq!(lexeme4, "\"say \\\"hello\\\"\"");

    let source_map5 = Rc::new(SourceMap::new("\"café 🦀\"".to_string()));
    let tokens5 = tokenize_all(source_map5.clone());
    assert_eq!(tokens5[0].token_type, TokenType::String);
    let lexeme5: String = tokens5[0].lexeme(&source_map5).iter().collect();
    assert_eq!(lexeme5, "\"café 🦀\"");

    let source_map6 = Rc::new(SourceMap::new("\"hello world! @#$%^&*()\"".to_string()));
    let tokens6 = tokenize_all(source_map6.clone());
    assert_eq!(tokens6[0].token_type, TokenType::String);
    let lexeme6: String = tokens6[0].lexeme(&source_map6).iter().collect();
    assert_eq!(lexeme6, "\"hello world! @#$%^&*()\"");

    let source_map7 = Rc::new(SourceMap::new("\"path\\\\to\\\\file\"".to_string()));
    let tokens7 = tokenize_all(source_map7.clone());
...
fn test_operator_lexeme_content() {
    ...
    for (op_str, expected_type) in operators {
        let source_map = Rc::new(SourceMap::new(op_str.to_string()));
        let tokens = tokenize_all(source_map.clone());
...
fn test_keyword_lexeme_token_contentt() {
    ...
    for (keyword_str, expected_type) in keywords {
        let source_map = Rc::new(SourceMap::new(keyword_str.to_string()));
        let tokens = tokenize_all(source_map.clone());
        assert_eq!(tokens[0].token_type, expected_type);
        let lexeme: String = tokens[0].lexeme(&source_map).iter().collect();
...
fn test_line_and_column_tracking() {
    let source_map = Rc::new(SourceMap::new("first\nsecond line".to_string()));
    let tokens = tokenize_all(source_map.clone());
